<template>
  <div class="app-container">
    <el-form :model="queryParams" size="small" :inline="true">
      ...
      <el-form-item label="姓名" prop="contestName">
        <el-input 
    ...
    >
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button status="primary" @click="handleEdt(row.enrollNo)"
            >审核</vxe-button
          >
          <vxe-button  @click="handleDet(row.enrollNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listContestenroll, delContestenroll, exptContestenroll } from '@/api/labsys/contestenroll'
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      exptContestenroll(this.pageParam).then((response) => {
    ...
    checkStateFormatter(value){
      /*console.log()
      console.log(111)*/
        if(value.row.checkState === '1'){
          return '通过';
        } else {
          return '未通过';
        }
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listContestenroll(this.pageParam).then((response) => {
