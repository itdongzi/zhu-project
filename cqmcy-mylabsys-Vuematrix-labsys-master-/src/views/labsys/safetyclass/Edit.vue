...
<script>
  import { getSafetyclass, addSafetyclass, uptSafetyclass } from '@/api/labsys/safetyclass'
...
export default {
    ...
    doOk() {
      ...
      this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            addSafetyclass(that.form).then((response) => {
              that.$layer.msg(response.msg);
              that.$parent.getDataSource();
              that.$layer.close(that.layerid);
            });
          } else {
            uptSafetyclass(that.form).then((response) => {
                that.$layer.msg(response.msg)
                that.$parent.getDataSource()
                that.$layer.close(that.layerid)
            });
  ...
  mounted() {
    if (this.id !== "") {
      const that = this;
      getSafetyclass(this.id).then(response => {
