...
<script>
  import { listSectionsinfo, delSectionsinfo, exptSectionsinfo } from '@/api/labsys/sectionsinfo'
...
export default {
  ...
  methods: {
    ...
    handleDel() {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          delSectionsinfo(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptSectionsinfo(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listSectionsinfo(this.pageParam).then((response) => {
