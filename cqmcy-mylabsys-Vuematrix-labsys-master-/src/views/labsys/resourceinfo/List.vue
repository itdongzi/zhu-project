<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="rescName" title="资源名称"></vxe-table-column>
      <vxe-table-column field="classNo" title="资源分类"></vxe-table-column>
      <vxe-table-column
        field="shareType"
        title="共享"
...
<script>
  import { listResourceinfo, delResourceinfo, exptResourceinfo } from '@/api/labsys/resourceinfo'
...
export default {
  ...
  methods: {
    ...
    handleDel() {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          delResourceinfo(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptResourceinfo(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listResourceinfo(this.pageParam).then((response) => {
