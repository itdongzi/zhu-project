<template>
  <div class="app-container">
    <el-form :model="queryParams" size="small" :inline="true">
      <el-form-item label="考核人员" prop="userName">
        <el-input
          v-model="queryParams.userName"
          placeholder="请输入考核人员姓名"
          clearable
    ...
    >
      ...
      <vxe-table-column field="userNo" title="考核人员"></vxe-table-column>
      <vxe-table-column field="paperNo" title="考核试卷"></vxe-table-column>
      <vxe-table-column
        field="testDate"
        title="考核时间"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.testNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.testNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listSafetytest, delSafetytest, exptSafetytest } from '@/api/labsys/safetytest'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.testNo);
          });
          delSafetytest(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptSafetytest(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listSafetytest(this.pageParam).then((response) => {
