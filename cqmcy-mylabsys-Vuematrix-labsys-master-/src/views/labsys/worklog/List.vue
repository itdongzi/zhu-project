<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="roomNo" title="实训室名称"></vxe-table-column>
      <vxe-table-column field="userNo" title="姓名"></vxe-table-column>
      <vxe-table-column
        field="workTitle"
        title="标题"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.workNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.workNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listWorklog, delWorklog, exptWorklog } from '@/api/labsys/worklog'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.workNo);
          });
          delWorklog(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptWorklog(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listWorklog(this.pageParam).then((response) => {
