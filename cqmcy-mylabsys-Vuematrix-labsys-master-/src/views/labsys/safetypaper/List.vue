<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="paperTitle" title="试卷名称"></vxe-table-column>
      <vxe-table-column field="paperType" title="试卷类别"></vxe-table-column>
      <vxe-table-column
        field="paperDuration"
        title="时长"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.paperNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.paperNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listSafetypaper, delSafetypaper, exptSafetypaper } from '@/api/labsys/safetypaper'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.paperNo);
          });
          delSafetypaper(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptSafetypaper(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listSafetypaper(this.pageParam).then((response) => {
