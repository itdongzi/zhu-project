<template>
  <div class="wrapbox">
    <el-descriptions title="安全试卷信息">
      <el-descriptions-item label="试卷名称">{{model.paperTitle}}</el-descriptions-item>
      <el-descriptions-item label="类别">{{model.paperType}}</el-descriptions-item>
      <el-descriptions-item label="考试时长">{{model.paperDuration}}</el-descriptions-item>
      <el-descriptions-item label="试卷地址">{{model.paperUrl}}</el-descriptions-item>
      <el-descriptions-item label="试卷简介">{{model.paperDesc}}</el-descriptions-item>
      <el-descriptions-item label="备注">{{model.comments}}</el-descriptions-item>
    </el-descriptions>
    <el-divider></el-divider>
    <div style="text-align:center;"><el-button size="small" @click="doClose">关闭</el-button></div>
  </div>
</template>

<script>
  import { getSafetypaper } from '@/api/labsys/safetypaper'

export default {
  name: "Detail",
  props: {
    layerid: {
      // 自动注入的layerid
      type: String,
      default: "",
    },
    id: {
      // 传递的数据
      type: String,
      default: "",
    },
    lydata: {
      type: Object,
      default: () => {
        return {};
      },
    },
  },
  data() {
    return {
      model: {
        paperNo: "0",
        paperTitle: "",
        paperType: "0",
        paperDuration: "0",
        paperUrl: "0",
        paperDesc: "0",
        shareType: "0",
        checkState: "1",
        comments: "",
      },
    };
  },
  methods: {
    doClose() {
      this.$layer.close(this.layerid)
    },
  },
  mounted() {
    if (this.id !== "") {
      const that = this;
      getSafetypaper(this.id).then(response => {
        that.model = response.data
      })
    }
  },
};
</script>

<style scoped lang="scss">
.wrapbox {
  margin: 0;
  padding: 30px;
  width: 100%;
}
</style>