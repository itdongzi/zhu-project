...
<script>
  import { getResourceclass, addResourceclass, uptResourceclass } from '@/api/labsys/resourceclass'
...
export default {
    ...
    doOk() {
      ...
      this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            addResourceclass(that.form).then((response) => {
              that.$layer.msg(response.msg);
              that.$parent.getDataSource();
              that.$layer.close(that.layerid);
            });
          } else {
            uptResourceclass(that.form).then((response) => {
                that.$layer.msg(response.msg)
                that.$parent.getDataSource()
                that.$layer.close(that.layerid)
            });
  ...
  mounted() {
    if (this.id !== "") {
      const that = this;
      getResourceclass(this.id).then(response => {
