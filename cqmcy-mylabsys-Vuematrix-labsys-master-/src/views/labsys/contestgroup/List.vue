<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="groupName" title="小组名称"></vxe-table-column>
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.groupNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.groupNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listContestgroup, delContestgroup, exptContestgroup } from '@/api/labsys/contestgroup'
...
export default {
  ...
  methods: {
    ...
    handleDel() {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          delContestgroup(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptContestgroup(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listContestgroup(this.pageParam).then((response) => {
