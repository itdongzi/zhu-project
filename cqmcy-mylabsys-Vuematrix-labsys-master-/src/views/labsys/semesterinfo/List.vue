...
<script>
  import { listSemesterinfo, delSemesterinfo, exptSemesterinfo } from '@/api/labsys/semesterinfo'
...
export default {
  ...
  methods: {
    ...
    handleDel() {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          delSemesterinfo(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptSemesterinfo(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listSemesterinfo(this.pageParam).then((response) => {
