...
<script>
  import { getSemesterinfo, addSemesterinfo, uptSemesterinfo } from '@/api/labsys/semesterinfo'
...
export default {
    ...
    doOk() {
      ...
      this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            addSemesterinfo(that.form).then((response) => {
              that.$layer.msg(response.msg);
              that.$parent.getDataSource();
              that.$layer.close(that.layerid);
            });
          } else {
            uptSemesterinfo(that.form).then((response) => {
                that.$layer.msg(response.msg)
                that.$parent.getDataSource()
                that.$layer.close(that.layerid)
            });
  ...
  mounted() {
    if (this.id !== "") {
      const that = this;
      getSemesterinfo(this.id).then(response => {
