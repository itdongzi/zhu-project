  ...
  <div class="app-container">
    <el-form :model="queryParams" size="small" :inline="true">
      <el-form-item label="成果名称" prop="achieTitle">
        <el-input
          v-model="queryParams.achieTitle"
          placeholder="请输入成果名称"
          clearable
    ...
    >
      ...
      <vxe-table-column field="achieTitle" title="成果名称"></vxe-table-column>
      <vxe-table-column field="classNo" title="成果分类"></vxe-table-column>
      <vxe-table-column
        field="achieDate"
        title="登记时间"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.achieNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.achieNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listAchieveinfo, delAchieveinfo, exptAchieveinfo } from '@/api/labsys/achieveinfo'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.achieNo);
          });
          delAchieveinfo(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptAchieveinfo(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listAchieveinfo(this.pageParam).then((response) => {
