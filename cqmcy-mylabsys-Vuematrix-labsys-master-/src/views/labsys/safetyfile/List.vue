<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="fileTitle" title="资料名称"></vxe-table-column>
      <vxe-table-column field="safeClass" title="资料类别"></vxe-table-column>
      <vxe-table-column field="fileFormat" title="资料格式"></vxe-table-column>
      <vxe-table-column
        field="checkState"
        title="审核状态"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.fileNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.fileNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listSafetyfile, delSafetyfile, exptSafetyfile } from '@/api/labsys/safetyfile'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.fileNo);
          });
          delSafetyfile(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptSafetyfile(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listSafetyfile(this.pageParam).then((response) => {
