<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="projectTitle" title="项目名称"></vxe-table-column>
      <vxe-table-column field="classNo" title="项目类型"></vxe-table-column>
      <vxe-table-column
        field="updateBy"
        title="上传人员"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <el-button type="primary" size="mini" plain @click="handleEdt(row.projectNo)"
            >编辑</el-button
          >
          <el-button type="primary" size="mini" plain @click="handleDet(row.projectNo)"
            >详细</el-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listProjectinfo, delProjectinfo, exptProjectinfo } from '@/api/labsys/projectinfo'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.projectNo);
          });
          delProjectinfo(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptProjectinfo(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listProjectinfo(this.pageParam).then((response) => {
