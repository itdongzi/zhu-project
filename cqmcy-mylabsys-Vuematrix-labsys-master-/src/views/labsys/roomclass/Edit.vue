...
<script>
  import { getRoomclass, addRoomclass, uptRoomclass } from '@/api/labsys/roomclass'
...
export default {
    ...
    doOk() {
      ...
      this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            addRoomclass(that.form).then((response) => {
              that.$layer.msg(response.msg);
              that.$parent.getDataSource();
              that.$layer.close(that.layerid);
            });
          } else {
            uptRoomclass(that.form).then((response) => {
                that.$layer.msg(response.msg)
                that.$parent.getDataSource()
                that.$layer.close(that.layerid)
            });
  ...
  mounted() {
    if (this.id !== "") {
      const that = this;
      getRoomclass(this.id).then(response => {
