<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="semeNo" title="学期"></vxe-table-column>
      <vxe-table-column
        field="userNo"
        title="预约人"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.orderNo)"
          >审核</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.orderNo)"
          >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
import { listRoomorders, delRoomorders, exptRoomorders } from '@/api/labsys/roomorders'
...
export default {
  ...
  methods: {
    ...
    handleDel() {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          delRoomorders(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptRoomorders(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listRoomorders(this.pageParam).then((response) => {
