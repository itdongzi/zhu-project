<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="certeName" title="证书名称"></vxe-table-column>
      <vxe-table-column field="cuserName" title="人员姓名"></vxe-table-column>
      <vxe-table-column
        field="certeDate"
        title="获证时间"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.certeNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.certeNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listSafetycerte, delSafetycerte, exptSafetycerte } from '@/api/labsys/safetycerte'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.certeNo);
          });
          delSafetycerte(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptSafetycerte(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listSafetycerte(this.pageParam).then((response) => {
