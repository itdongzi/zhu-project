...
<script>
  import { getResourcefiles, addResourcefiles, uptResourcefiles } from '@/api/labsys/resourcefiles'
...
export default {
    ...
    doOk() {
      ...
      this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            addResourcefiles(that.form).then((response) => {
              that.$layer.msg(response.msg);
              that.$parent.getDataSource();
              that.$layer.close(that.layerid);
            });
          } else {
            uptResourcefiles(that.form).then((response) => {
                that.$layer.msg(response.msg)
                that.$parent.getDataSource()
                that.$layer.close(that.layerid)
            });
  ...
  mounted() {
    if (this.id !== "") {
      const that = this;
      getResourcefiles(this.id).then(response => {
