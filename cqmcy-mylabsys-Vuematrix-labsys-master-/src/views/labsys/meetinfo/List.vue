<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="meetName" title="会议室名称"></vxe-table-column>
      <vxe-table-column field="areaNo" title="楼栋区域"></vxe-table-column>
      <vxe-table-column
        field="meetAdmin"
        title="管理员"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.meetNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.meetNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
import { listMeetinfo, delMeetinfo, exptMeetinfo } from "@/api/labsys/meetinfo";
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？", (layerid) => {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.meetNo);
          });
          delMeetinfo(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptMeetinfo(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listMeetinfo(this.pageParam).then((response) => {
