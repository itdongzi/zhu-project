<template>
  <div class="app-container">
    <el-form :model="queryParams" size="small" :inline="true">
      <el-form-item label="会议室" prop="meetName">
        <el-input
    ...
    >
      ...
      <vxe-table-column field="meetNo" title="会议室名称"></vxe-table-column>
      <vxe-table-column
        field="userName"
        title="预约人"
...
<script>
import { listMeetorders, delMeetorders, exptMeetorders } from '@/api/labsys/meetorders'
...
export default {
  ...
  methods: {
    ...
    handleDel() {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          delMeetorders(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptMeetorders(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listMeetorders(this.pageParam).then((response) => {
