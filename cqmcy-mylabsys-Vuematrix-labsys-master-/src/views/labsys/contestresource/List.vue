<template>
  <div class="app-container">
    <el-form :model="queryParams" size="small" :inline="true">
      <el-form-item label="资源名称" prop="roomName">
        <el-input
          v-model="queryParams.rescTitle"
          placeholder="请输入资源名称"
          clearable
    ...
    >
      ...
      <vxe-table-column field="rescTitle" title="资源名称"></vxe-table-column>
      <vxe-table-column field="contentNo" title="竞赛名称"></vxe-table-column>
      <vxe-table-column field="rescExt" title="资源类别"></vxe-table-column>
      <vxe-table-column
        field="userNo"
        title="上传人员"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.rescNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.rescNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listContestresource, delContestresource, exptContestresource } from '@/api/labsys/contestresource'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$layer.confirm("您确定要删除所选的记录吗？",(layerid)=> {
          ...
          selectedRecords.map(function (item) {
            selectedRowKeys.push(item.rescNo);
          });
          delContestresource(selectedRowKeys).then((response) => {
            that.getDataSource();
    ...
    handleExport() {
      ...
      exptContestresource(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listContestresource(this.pageParam).then((response) => {
