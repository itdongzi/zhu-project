  ...
  <div class="app-container">
    <el-form :model="queryParams" size="small" :inline="true">
      <el-form-item label="姓名" prop="userCnname">
        <el-input
          v-model="queryParams.userCnname"
          placeholder="请输入姓名"
          clearable
    ...
    >
      ...
      <vxe-table-column field="userCnname" title="姓名"></vxe-table-column>
      <vxe-table-column field="userType" title="类别"></vxe-table-column>
      <vxe-table-column
        field="deptNo"
        title="部门"
      ...
      <vxe-table-column title="操作">
        <template v-slot="{ row }">
          <vxe-button type="text" @click="handleEdt(row.userNo)"
            >编辑</vxe-button
          >
          <vxe-button type="text" @click="handleDet(row.userNo)"
            >详细</vxe-button
          >
        </template>
      </vxe-table-column>
...
<script>
  import { listRuserinfo, delRuserinfo, exptRuserinfo } from '@/api/system/ruserinfo'
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      exptRuserinfo(this.pageParam).then((response) => {
    ...
    getDataSource() {
      ...
      this.loading = true;
      this.pageParam.dataParams = this.queryParams;
      listRuserinfo(this.pageParam).then((response) => {
